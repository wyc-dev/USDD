...
contract USDD is ERC20, Ownable, ReentrancyGuard {
     ...
     * @dev Initially set to the contract deployer; can be updated by the owner to a separate treasury
     */
    ...
    /**
     * @notice Emitted when early unstake fee is updated
     * @param newFEE New maximum fee in basis points
     */
    event UnstakeFEEUpdated(uint256 indexed newFEE);

    /**
     * @notice Emitted when boundary amount threshold is updated
    ...
    /**
     * @notice Emitted when operation manager status is updated
    ...
    /**
     * @notice Emitted when stuck assets are withdrawn by owner
    ...
    /**
     * @dev Modifier restricting access to owner or authorized operation managers
     */
    ...
    /**
     * @notice Owner-only function to set operation manager status for an address
     * @dev Operation managers can fulfill redemptions using their own funds
    ...
    /**
     * @notice Deposit USDC to mint equal amount of USDD (1:1)
     * @dev All received USDC is immediately transferred to the vault address. Optional referrer can be set once.
    ...
    function depositUSDC(uint256 amount, address referrer) external nonReentrant {
        ...
        IERC20(USDC_BASE).safeTransferFrom(sender, address(this), amount);
    ...
    /**
     * @notice Fulfill a pending redemption by transferring USDC from the caller's address to the investor
     * @dev Restricted to owner or authorized operation managers. Caller must have approved this contract for USDC.
     *      Burns the escrowed USDD after successful transfer. This allows separate fund management per operator.
     * @param investor Address of the investor to fulfill redemption for
     */
    function fulfillRedemption(address investor) external onlyAuthorizedRedeemer nonReentrant {
        ...
        IERC20(USDC_BASE).safeTransferFrom(redeemer, investor, amount);
    ...
    /**
     * @notice Stake full amount of USDD for yield
    ...
    /**
     * @notice Unstake full staked amount, claim rewards, and pay any applicable fees
    ...
    function unstakeUSDD() external nonReentrant {
        ...
        // 1. Calculate and mint reward (multiplication first to preserve precision)
        uint256 rewardToMint = (amount * stakingAPY * timeStaked) / (BPS_DENOMINATOR * SECONDS_PER_YEAR);
        if (rewardToMint > 0) {
            _mint(sender, rewardToMint);
        }

        // 2. Early unstake fee (VIP exempt) - multiplication first
        ...
        // 3. Small-amount fee - multiplication first
        uint256 smallFeeAmount = 0;
        if (amount < boundaryAmount && stakingAPY > 0) {
            smallFeeAmount = (amount * stakingAPY) / BPS_DENOMINATOR;
        }

        // 4. Referral reward 2: on unstake (1% of principal) - multiplication first
     ...
     * @param account Address to query
     * @return Early unstake fee in USDD
     */
     ...
     */
    function pendingUnstakeFee(address account) external view returns (uint256) {
